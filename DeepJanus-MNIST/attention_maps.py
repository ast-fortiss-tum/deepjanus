...
from config import EXTENT_LOWERBOUND, \
    EXTENT_UPPERBOUND
...
def AM_darken_attention_pixels_mth1(images, x_patch_size, y_patch_size, svg_path):
    ...
    for i in range(images.shape[0]):
        ...
        x, y = get_attetion_region_and_darken_pixel_mth1(xai[i], images[i], x_patch_size,
...
def AM_darken_attention_pixels_mth2(images, x_patch_size, y_patch_size, svg_path, number_of_regions):
    ...
    for i in range(images.shape[0]):
        ...
        # x, y = get_attetion_region(xai[i], images[i], x_patch_size, y_patch_size) #Getting coordinates of the highest attetion region (patch) reference point
        list_of_regions = get_attetion_region_and_darken_pixel_mth2(xai[i], images[i], x_patch_size, y_patch_size,
                                                                    number_of_regions)  # Getting coordinates of the highest attetion region (patch) reference point
        copy_orig_image = copy.deepcopy(images[i])
...
def AM_get_attetion_svg_points_image(image, x_patch_size, y_patch_size, model):  # images should have the shape: (28, 28)
    ...
    x, y, elapsed_time = get_attetion_region(xai[0], images[0], x_patch_size,
                                             y_patch_size)  # Getting coordinates of the highest attetion region (patch) reference point
...
def AM_get_attetion_svg_points_images_mth2(images, sqr_size, svg_path):
    ...
    for i in range(images.shape[0]):
        ...
        position, elapsed_time = get_attetion_region_mth4(xai[i], controlPoints,
                                                          sqr_size)  # Getting coordinates of the highest attetion region (patch) reference point
...
def AM_get_attetion_svg_points_images_mth5(images, number_of_points, svg_path):
    ...
    for i in range(images.shape[0]):
        ...
        position = get_attetion_region_mth5(xai[i], controlPoints,
...
def apply_mutoperator_attention_2(input_img, svg_path, extent, model):
    ...
    #                                                                                                  svg_path)
    list_of_points_inside_square_attention_patch, elapsed_time = AM_get_attetion_svg_points_images_mth5(input_img, 2,
                                                                                                        svg_path)

    list_of_mutated_coordinates_string = apply_displacement_to_mutant_2(list_of_points_inside_square_attention_patch[0],
                                                                        extent)
...
def mutate_attention(attention_map, svg_desc, mutation_extent, square_size=2, number_of_points=2):
    ...
    list_of_points_close_to_square_attention_patch = get_svg_points_distance_mth_integration(attention_map, square_size,
                                                                                             square_size, svg_desc,
                                                                                             number_of_points)
...
def get_svg_points_distance_mth_integration(xai, x_patch_size, y_patch_size, svg_path, number_of_points):
    ...
    for i in range(xai.shape[0]):
        ...
        controlPoints = [(float(i[0]), float(i[1])) for i in ControlPoints]
        if len(ControlPoints) != 0:
            x, y = get_attetion_region(xai[i], None, x_patch_size,
                                       y_patch_size)  # Getting coordinates of the highest attetion region (patch) reference point
            list_of_ControlPointsCloseToRegion = getControlPointsCloseToRegion(x, y, x_patch_size, y_patch_size,
                                                                               controlPoints,
                                                                               number_of_points)  # Getting all the points inside the highest attetion patch
...
def generate_mutant(image, svg_path, extent, square_size, number_of_points, mutation_method, ATTENTION_METHOD):
    ...
    if mutation_method == True:
        ...
        if list_of_svg_points != None and ("C" in mutante_digit_path) and ("M" in mutante_digit_path):
            rast_nparray = rasterization_tools.rasterize_in_memory(
                vectorization_tools.create_svg_xml(mutante_digit_path))
...
def save_image(mutant_image_normal, mutant_image_att, xai_image, list_of_svg_points, iteration_list,
               fitness_function_att, prediction_function_att, fitness_function_normal, prediction_function_normal,
               number_of_mutations, folder_path, pred_normal, pred_att, ATTENTION_METHOD, square_size, iteration):
    ...
    if ATTENTION_METHOD == "mth5":
        for pos in list_of_svg_points[0]:
            ...
            rect = patches.Rectangle((x - (square_size / 2), y - (square_size / 2)), square_size, square_size,
...
def save_boxPlots_adaptive2(a, a_adp, b, b_adp, c, c_adp, d, d_adp, folder_path, ext, number_of_mutations,
...
def save_images(mutant_image_normal_list, mutant_image_att_list, xai_image_list, list_of_svg_points_list, iteration_list,
                fitness_function_att, prediction_function_att, fitness_function_normal, prediction_function_normal,
                number_of_mutations, folder_path, pred_normal_list, pred_att_list, ATTENTION_METHOD, square_size,
                square_att_coordinates_list, original_svg_points_list, mutated_points_att_list_numeric, ext_att_list,
                ext_normal_list):
    ...
    for img_index in range(len(iteration_list)):
        if img_index % print_interval == 0 or img_index == len(iteration_list) - 1:
            ...
            if ATTENTION_METHOD == "mth5":
                ...
                rect = patches.Rectangle((square_coordinate_X + 0.5, square_coordinate_Y + 0.5), square_size,
                                         square_size, linewidth=1, edgecolor='r', facecolor='none')
                ax2.add_patch(rect)

            ax_fitness = fig.add_subplot(gs[1, :])
            ax_fitness.plot(iteration_list, fitness_function_att, "b", label="Attention Algorithm")
            ax_fitness.plot(iteration_list[img_index], fitness_function_att[img_index], marker="o",
                            markeredgecolor="blue")
            ax_fitness.plot(iteration_list, fitness_function_normal, "r", label="Normal Algorithm")
            ax_fitness.plot(iteration_list[img_index], fitness_function_normal[img_index], marker="o",
                            markeredgecolor="blue")
            ax_fitness.set_title("Fitness ff2 vs Iteration")
            ax_fitness.set_xlabel("Iteration")
            ax_fitness.set_ylabel("Fitness ff2")
            ...
            ax_predictions = fig.add_subplot(gs[2, :])
            ax_predictions.plot(iteration_list, prediction_function_att, "b", label="Attention Algorithm")
            ax_predictions.plot(iteration_list[img_index], prediction_function_att[img_index], marker="o",
                                markeredgecolor="blue")
            ax_predictions.plot(iteration_list, prediction_function_normal, "r", label="Normal Algorithm")
            ax_predictions.plot(iteration_list[img_index], prediction_function_normal[img_index], marker="o",
                                markeredgecolor="blue")
            ax_predictions.set_title("Mutant Prediction Probablity vs Iteration")
            ax_predictions.set_xlabel("Iteration")
            ax_predictions.set_ylabel("Mutant Prediction Probablity")
            ...
            plt.tight_layout()
            plt.savefig(folder_path + "/iteration=" + str(img_index) + "_predATR=" + str(
                pred_att_list[img_index]) + "_predNOR=" + str(np.argmax(pred_normal_list[img_index])))
            plt.cla()
            plt.close(fig)


def save_images_adaptive(mutant_image_normal_list, mutant_image_att_list, mutant_image_att_adaptive_list,
                         mutant_digit_normal_adaptive_list, xai_image_list, xai_image_adaptive_list,
                         list_of_svg_points_list, list_of_svg_points_adaptive_list, iteration_list, fitness_function_att,
                         fitness_function_att_adaptive, prediction_function_att, prediction_function_att_adaptive,
                         fitness_function_normal, fitness_function_normal_adaptive, prediction_function_normal,
                         prediction_function_normal_adaptive, number_of_mutations, folder_path, pred_normal_list,
                         ...
                         original_svg_points_adaptive_list, mutated_points_att_list_numeric,
                         mutated_points_att_adaptive_list_numeric, ext_att_list, ext_att_adaptive_list, ext_normal_list,
    ...
    for img_index in range(len(iteration_list)):
        if img_index % print_interval == 0 or img_index == len(iteration_list) - 1:
            ...
            ax_mutant_normal_adaptive_image.imshow(mutant_digit_normal_adaptive_list[img_index].reshape(28, 28),
                                                   cmap="gray")
            ax_mutant_normal_adaptive_image.set_title("Normal Adpt Pred = " + str(pred_normal_list[img_index]),
                                                      color="yellow")
            ...
            ax_mutant_att_adaptive_image.set_title("Att ADAPTIVE Pred= " + str(pred_att_adaptive_list[img_index]),
                                                   color="green")
            ...
            if ATTENTION_METHOD == "mth5":
                ...
                for pos in list_of_svg_points_adaptive_list[img_index][0]:
                    ...
                    x_rounded = round(x, 2)
                    y_rounded = round(y, 2)
                    ax_xai_att_adaptive_image.annotate("(" + str(x_rounded) + "," + str(y_rounded) + ")",
                                                       (pos[0], pos[1]))
                    rect = patches.Rectangle((x - (square_size / 2), y - (square_size / 2)), square_size, square_size,
                                             linewidth=1, edgecolor='r', facecolor='none')
                ...
                # rect = patches.Rectangle((square_coordinate_X-(square_size/2), square_coordinate_Y-(square_size/2)), square_size, square_size, linewidth=1, edgecolor='r', facecolor='none')
                rect_adaptive = patches.Rectangle(
                    (square_coordinate_X_adaptive + 0.5, square_coordinate_Y_adaptive + 0.5), square_size, square_size,
                    linewidth=1, edgecolor='r', facecolor='none')
                ...
                # ADAPTIVE PART
                # Printing the annotations of all returned SVG points that are closer to the highest attention path
                for pos, prob in zip(original_svg_points_adaptive_list[img_index],
                                     list_of_svg_points_adaptive_list[img_index]):
                    x = pos[0]
                    y = pos[1]
                    x_rounded = round(x, 2)
                    y_rounded = round(y, 2)
            ...
            ax_fitness.plot(iteration_list[img_index], fitness_function_att_adaptive[img_index], marker="o",
                            markeredgecolor="blue")
            # NORMAL PART
            ax_fitness.plot(iteration_list, fitness_function_normal, "r", label="Normal Algorithm")
            ax_fitness.plot(iteration_list[img_index], fitness_function_normal[img_index], marker="o",
            ...
            ax_fitness.plot(iteration_list[img_index], fitness_function_normal_adaptive[img_index], marker="o",
                            markeredgecolor="blue")
            ax_fitness.set_title("Fitness ff2 vs Iteration")
            ax_fitness.set_xlabel("Iteration")
            ax_fitness.set_ylabel("Fitness ff2")
            ...
            ax_predictions.plot(iteration_list[img_index], prediction_function_att[img_index], marker="o",
                                markeredgecolor="blue")
            # ADAPTIVE PART
            ax_predictions.plot(iteration_list, prediction_function_att_adaptive, "g",
                                label="Attention Adaptive Algorithm")
            ax_predictions.plot(iteration_list[img_index], prediction_function_att_adaptive[img_index], marker="o",
                                markeredgecolor="blue")
            # NORMAL PART
            ax_predictions.plot(iteration_list, prediction_function_normal, "r", label="Normal Algorithm")
            ax_predictions.plot(iteration_list[img_index], prediction_function_normal[img_index], marker="o",
            ...
            ax_predictions.plot(iteration_list[img_index], prediction_function_normal_adaptive[img_index], marker="o",
                                markeredgecolor="blue")
            ax_predictions.set_title("Mutant Prediction Probablity vs Iteration")
            ax_predictions.set_xlabel("Iteration")
            ax_predictions.set_ylabel("Mutant Prediction Probablity")
            ...
            ax_extent.plot(iteration_list[img_index], ext_att_adaptive_list[img_index], marker="o",
                           markeredgecolor="blue")
            # NORMAL PART
            ax_extent.plot(iteration_list, ext_normal_list, "r", label="Extent Normal Algorithm")
            ax_extent.plot(iteration_list[img_index], ext_normal_list[img_index], marker="o", markeredgecolor="blue")
            ...
            ax_extent.plot(iteration_list[img_index], ext_normal_adaptive_list[img_index], marker="o",
                           markeredgecolor="blue")
            ax_extent.set_title("Extents vs Iteration")
            ax_extent.set_xlabel("Iteration")
            ax_extent.set_ylabel("Extent Value")
            ...
            plt.tight_layout()
            plt.savefig(folder_path + "/iteration=" + str(img_index) + "_predATT=" + str(
                pred_att_list[img_index]) + "_predATTADP=" + str(pred_att_adaptive_list[img_index]) + "_predNOR=" + str(
                pred_normal_list[img_index]) + "_predNORADP=" + str(pred_normal_adaptive_list[img_index]))
...
def option1():
    ...
    for image_index in range(images.shape[0]):
        ...
        for iteration in range(0, 19):
            ...
            list_of_points_inside_square_attention_patch, elapsed_time = AM_get_attetion_svg_points_images_mth1(image,
                                                                                                                square_size,
                                                                                                                square_size,
                                                                                                                get_svg_path(
                                                                                                                    image[
            ...
            if prediction != prediction_mnist_data:
                ...
                #     ax[2].add_patch(rect)
                # plt.tight_layout()
                plt.savefig("./mutants/mutant_img=" + str(image_index) + "_ext=" + str(extent) + "_sqr=" + str(
                    square_size) + "_Pred=" + str(prediction[0]) + "_PredOrig=" + str(
                    prediction_mnist_data[0]) + "_lab=" + str(label) + "_" + str(iteration) + '.png')
                plt.cla()
            ...
            for METHOD in METHOD_LIST:
                ...
                for ATT_METH in ATT_METH_LIST:
                    ...
                    while (iteration < number_of_mutations):
                        ...
                        if ATT_METH == True:
                            mutant_digit, list_of_svg_points, xai = generate_mutant(
                                input_reshape_images_reverse(digit_reshaped), extent, square_size, number_of_points,
                                ATT_METH)
                            ...
                            mutant_digit = generate_mutant(input_reshape_images_reverse(digit_reshaped), extent,
                                                           square_size, number_of_points, ATT_METH)
                        ...
                        if pred_input_mutant[0] != LABEL:
                            save_image(digit_reshaped, mutant_digit, xai, list_of_svg_points, iteration_list,
                                       fitness_function, prediction_function, number_of_mutations, DST,
                                       pred_input_mutant[0])
                            with open(csv_path, "a") as f1:
                                writer = csv.writer(f1)
                                writer.writerow(
                                    [METHOD, LABEL, OCCURENCE, ATT_METH, iteration, pred_class_mutant[0][LABEL]])
                            break
    ...
    for METHOD in METHOD_LIST:
        ...
        for image_index in range(images.shape[0]):
            ...
            for repetition in range(1, number_of_repetitions + 1):
                ...
                while (iteration < number_of_mutations):
                    ...
                    if miss_classification_found_att == False:
                        ...
                        mutant_digit_att_candidate, list_of_svg_points, xai, point_mutated, square_att_coordinates, original_svg_points, svg_path_att_mth_candidate = generate_mutant(
                            input_reshape_images_reverse(digit_reshaped_1), svg_path_att_mth, ext_att, square_size,
                            number_of_points, True, ATTENTION_METHOD)
                    ...
                    if miss_classification_found_normal == False:
                        ...
                        if iteration == 1:
                            ...
                            pred_input_mutant_normal = np.argmax(model.predict(mutant_digit_normal))
                        ...
                        pred_input_mutant_normal_candidate = np.argmax(model.predict(mutant_digit_normal_candidate))
                    ...
                    pred_input_mutant_att_list.append(pred_input_mutant_att)
                    pred_input_mutant_normal_list.append(pred_input_mutant_normal)
                    ...
                    if pred_input_mutant_att[0] != LABEL:
                        if miss_classification_found_att == False:
                            ...
                            with open(csv_path, "a") as f1:
                                writer = csv.writer(f1)
                                writer.writerow(
                                    [image_index, "ATTENTION", METHOD, LABEL, np.argmax(pred_input_mutant_att[0]),
                    ...
                    if pred_input_mutant_normal != LABEL:
                        if not miss_classification_found_normal:
                            ...
                            with open(csv_path, "a") as f1:
                                writer = csv.writer(f1)
                                writer.writerow([image_index, "NORMAL", METHOD, LABEL, pred_input_mutant_normal,
                ...
                if True:
                    if SAVE_IMAGES == True and list_of_svg_points != None:
                        ...
                        save_images(mutant_digit_normal_list, mutant_digit_att_list, xai_images_list,
                                    list_of_svg_points_list, iteration_list, fitness_function_att,
                                    prediction_function_att, fitness_function_normal, prediction_function_normal,
                                    number_of_mutations, folder_path, pred_input_mutant_normal_list,
                                    pred_input_mutant_att_list, ATTENTION_METHOD, square_size,
                                    square_att_coordinates_list, original_svg_points_list,
                                    mutated_points_att_list_numeric, ext_att_list, ext_normal_list)
            ...
            with open(csv_path_3, "a") as f1:
                writer = csv.writer(f1)
                writer.writerow([image_index, LABEL, iterations_mean_att, iterations_mean_normal, iterations_std_att,
                                 iterations_std_normal, number_of_miss_classification_att,
                                 number_of_miss_classification_normal])

            save_boxPlots(iterations_mean_att_list, iterations_mean_normal_list, number_of_miss_classification_att_list,
                          number_of_miss_classification_normal_list, DST, EXTENT, NUMBER_OF_MUTATIONS,
                          NUMBER_OF_REPETITIONS)
...
def option4():
    ...
    # list_of_n = [10, 20, 30]
    list_of_n = [i for i in list(range(8001)) if
                 (i % 50 == 0 and i > 0 and i <= 1000) or (i == 2000) or (i == 4000) or (i == 8000)]
    print(list_of_n)
...
def how_to_use_Vincenzo_fuctions():
    ...
    for i in range(attention_maps.shape[0]):
        ...
        # TODO: Core part
        mutant_vector_rasterized, list_of_svg_points, xai, point_mutated, square_att_coordinates, original_svg_points, svg_path_att_mth_candidate = generate_mutant(
            image, svg_path_att_mth, extent, square_size, number_of_points, True, "distances")
        ...
        pred_input_mutant_att_candidate = model.predict_classes(
            digit_reshaped_and_normalized)  # Predict and Predict_classes only accept NORMALIZED images with shape: (N, 28, 28, 1) where N:Number Of Images >= 1
        pred_class_mutant_att_candidate = model.predict(
            digit_reshaped_and_normalized)  # Predict and Predict_classes only accept images NORMALIZED with shape: (N, 28, 28, 1) where N:Number Of Images >= 1
        print("pred_input_mutant_att_candidate", pred_input_mutant_att_candidate)
        print("pred_class_mutant_att_candidate", pred_class_mutant_att_candidate)
        # Plotting the images
        fig = plt.figure(figsize=(9, 10))
...
def Comparison_Script_Attention_vs_Normal_Mutation_vs_adaptive():
    ...
    with open(csv_path_2, append_write) as f1:
        writer = csv.writer(f1)
        writer.writerow(["IMG_Index", "Label", "Repetition", "Seed", "#Iterations_Att", "#Iterations_att_adaptive",
                         "#Iterations_Normal", "Winner Method"])
    ...
    with open(csv_path_3, append_write) as f1:
        writer = csv.writer(f1)
        writer.writerow(["IMG_Index", "Label", "Its_Mean_Att", "Its_Mean_Att_adaptive", "Its_Mean_Normal", "Its_Std_Att",
                         "Its_Std_Att_adaptive", "Its_Std_Normal", "#MissClass_found_att",
                         "#MissClass_found_att_adaptive", "#MissClass_found_Normal"])
    ...
    for METHOD in METHOD_LIST:
        ...
        for image_index in range(images.shape[0]):
                ...
                while (iteration < number_of_mutations_adaptive):
                    if iteration == number_of_mutations_adaptive - 1:
                        if adaptive_mutation_already_applied == False:
                            if fitness_function_att_list[-1] < FITNESS_THRESHOLD_TO_GENERATE_MORE_MUTATIONS or \
                                    ...
                                    fitness_function_att_adaptive_list[
                                        -1] < FITNESS_THRESHOLD_TO_GENERATE_MORE_MUTATIONS:
                    ...
                    if miss_classification_found_att_adaptive == False:
                        ...
                        if iteration == 1:
                            ...
                            # Generating Mutant Candidate
                        mutant_digit_att_candidate_adaptive, list_of_svg_points_adaptive, xai_adaptive, point_mutated_adaptive, square_att_coordinates_adaptive, original_svg_points_adaptive, svg_path_att_mth_candidate_adaptive = generate_mutant(
                            input_reshape_images_reverse(digit_reshaped_3), svg_path_att_adaptive_mth, ext_att_adaptive,
                            square_size, number_of_points, True, ATTENTION_METHOD)
                        ...
                        # Analysing if the candidate is good
                        pred_input_mutant_att_candidate_adaptive = model.predict_classes(
                            mutant_digit_att_candidate_adaptive)
                        pred_class_mutant_att_candidate_adaptive = model.predict(mutant_digit_att_candidate_adaptive)
                        fitness_mutant_att_candidate_adaptive = evaluate_ff2(pred_class_mutant_att_candidate_adaptive,
                                                                             LABEL)
                    ...
                    if pred_input_mutant_att_adaptive[0] != LABEL:
                        if miss_classification_found_att_adaptive == False:
                            ...
                            with open(csv_path, "a") as f1:
                                writer = csv.writer(f1)
                                writer.writerow(
                                    [image_index, "ATTENTION ADAPTIVE", METHOD, LABEL, pred_input_mutant_att_adaptive[0],
                                     pred_class_mutant_att_adaptive[0][LABEL], iteration])
                ...
                if True:
                    if SAVE_IMAGES == True and list_of_svg_points != None and list_of_svg_points_adaptive != None:
                        folder_path = create_folder(DST, number_of_mutations, repetition, ext_att, ext_normal, LABEL,
                                                    ...
                                                    indices_choosen[image_index])
                        # save_image(mutant_digit_normal, mutant_digit_att, xai, list_of_svg_points, iteration_list, fitness_function_att, prediction_function_att, fitness_function_normal, prediction_function_normal, number_of_mutations, folder_path, pred_input_mutant_normal[0], pred_input_mutant_att[0], ATTENTION_METHOD, square_size, iteration)
                        save_images_adaptive(mutant_digit_normal_list, mutant_digit_att_list,
                                             mutant_digit_att_adaptive_list, xai_images_list, xai_images_adaptive_list,
                                             list_of_svg_points_list, list_of_svg_points_adaptive_list, iteration_list,
                                             fitness_function_att_list, fitness_function_att_adaptive_list,
                                             prediction_function_att_list, prediction_function_att_adaptive_list,
                                             fitness_function_normal_list, prediction_function_normal_list,
                                             number_of_mutations, folder_path, pred_input_mutant_normal_list,
                                             pred_input_mutant_att_list, pred_input_mutant_att_adaptive_list,
                                             ATTENTION_METHOD, square_size, square_att_coordinates_list,
                                             square_att_coordinates_adaptive_list, original_svg_points_list,
                                             original_svg_points_adaptive_list, mutated_points_att_list_numeric,
                                             mutated_points_att_adaptive_list_numeric, ext_att_list,
                                             ext_att_adaptive_list, ext_normal_list)
                        make_gif(folder_path, folder_path + "/gif")
                    ...
                    with open(csv_path_2, "a") as f1:
                        writer = csv.writer(f1)
                        writer.writerow([image_index, LABEL, repetition, seed, iterations_detection_att,
                                         iterations_detection_att_adaptive, iterations_detection_normal, method_winner])
            ...
            with open(csv_path_3, "a") as f1:
                writer = csv.writer(f1)
                writer.writerow(
                    [image_index, LABEL, iterations_mean_att, iterations_mean_att_adaptive, iterations_mean_normal,
                     iterations_std_att, iterations_std_att_adaptive, iterations_std_normal,
                     number_of_miss_classification_att, number_of_miss_classification_att_adaptive,
                     number_of_miss_classification_normal])

            save_boxPlots_adaptive(iterations_mean_att_list, iterations_mean_att_adaptive_list,
                                   iterations_mean_normal_list, number_of_miss_classification_att_list,
                                   number_of_miss_classification_att_adaptive_list,
                                   number_of_miss_classification_normal_list, DST, "ADAPTIVE", NUMBER_OF_MUTATIONS,
                                   NUMBER_OF_REPETITIONS)
            print("iterations_mean_att_list: ", iterations_mean_att_list)
...
def Comparison_Script_Attention_vs_Normal_Mutation_vs_adaptive_vs_normal_adaptive():
    ...
    with open(csv_path_2, append_write) as f1:
        writer = csv.writer(f1)
        writer.writerow(["IMG_Index", "Label", "Repetition", "Seed", "#Iterations_Att", "#Iterations_att_adaptive",
                         "#Iterations_Normal", "#Iterations_Normal_adaptive", "Winner Method"])
    ...
    with open(csv_path_3, append_write) as f1:
        writer = csv.writer(f1)
        writer.writerow(["IMG_Index", "Label", "Its_Mean_Att", "Its_Mean_Att_adaptive", "Its_Mean_Normal",
                         "Its_Mean_Normal_adaptive", "Its_Std_Att", "Its_Std_Att_adaptive", "Its_Std_Normal",
                         "Its_Std_Normal_Adaptive", "#MissClass_found_att", "#MissClass_found_att_adaptive",
                         "#MissClass_found_Normal", "#MissClass_found_Normal_adaptive"])
    ...
    for METHOD in METHOD_LIST:
        ...
        for image_index in range(images.shape[0]):
            ...
            for repetition in range(1, number_of_repetitions + 1):
                ...
                while (iteration < number_of_mutations_adaptive):
                    ...
                    if miss_classification_found_att == False:
                        ...
                        if iteration == 1:
                            ...
                            fitness_mutant_att = evaluate_ff2(pred_class_mutant_att, LABEL)
                            mutant_digit_att = digit_reshaped_1
                            pred_input_mutant_att = np.argmax(model.predict(mutant_digit_att),
                                                              axis=-1)  # model.predict_classes(mutant_digit_att)
                        # Generating Mutant Candidate
                        mutant_digit_att_candidate, list_of_svg_points, xai, point_mutated, square_att_coordinates, original_svg_points, svg_path_att_mth_candidate = generate_mutant(
                            input_reshape_images_reverse(digit_reshaped_1), svg_path_att_mth, ext_att, square_size,
                            number_of_points, True, ATTENTION_METHOD)
                    ...
                    if miss_classification_found_normal_adaptive == False:
                        ...
                        #     fitness_2 = fitness_2_tmp
                        if iteration == 1:
                            svg_path_normal_adaptive_mth = get_svg_path(
                                input_reshape_images_reverse(digit_reshaped_4)[0])
                        ...
                        mutant_digit_normal_candidate_adaptive, point_mutated_normal_adaptive, svg_path_normal_mth_candidate_adaptive = generate_mutant(
                            input_reshape_images_reverse(digit_reshaped_4), svg_path_normal_adaptive_mth,
                            ext_normal_adaptive, square_size, number_of_points, False, ATTENTION_METHOD)
                        pred_input_mutant_normal_candidate_adaptive = model.predict_classes(
                            mutant_digit_normal_candidate_adaptive)
                        pred_class_mutant_normal_candidate_adaptive = model.predict(
                            mutant_digit_normal_candidate_adaptive)
                        fitness_mutant_normal_candidate_adaptive = evaluate_ff2(
                            pred_class_mutant_normal_candidate_adaptive, LABEL)
                    ...
                    if pred_input_mutant_normal_adaptive[0] != LABEL:
                        if miss_classification_found_normal_adaptive == False:
                            ...
                            with open(csv_path, "a") as f1:
                                writer = csv.writer(f1)
                                writer.writerow(
                                    [image_index, "NORMAL ADAPTIVE", METHOD, LABEL, pred_input_mutant_normal_adaptive[0],
                                     pred_class_mutant_normal_adaptive[0][LABEL], iteration])
                ...
                if True:
                    if SAVE_IMAGES == True and list_of_svg_points != None and list_of_svg_points_adaptive != None:
                        ...
                        # save_image(mutant_digit_normal, mutant_digit_att, xai, list_of_svg_points, iteration_list, fitness_function_att, prediction_function_att, fitness_function_normal, prediction_function_normal, number_of_mutations, folder_path, pred_input_mutant_normal[0], pred_input_mutant_att[0], ATTENTION_METHOD, square_size, iteration)
                        save_images_adaptive(mutant_digit_normal_list, mutant_digit_att_list,
                                             mutant_digit_att_adaptive_list, mutant_digit_normal_adaptive_list,
                                             xai_images_list, xai_images_adaptive_list, list_of_svg_points_list,
                                             list_of_svg_points_adaptive_list, iteration_list, fitness_function_att_list,
                                             fitness_function_att_adaptive_list, prediction_function_att_list,
                                             prediction_function_att_adaptive_list, fitness_function_normal_list,
                                             fitness_function_normal_adaptive_list, prediction_function_normal_list,
                                             prediction_function_normal_adaptive_list, number_of_mutations, folder_path,
                                             pred_input_mutant_normal_list, pred_input_mutant_normal_adaptive_list,
                                             pred_input_mutant_att_list, pred_input_mutant_att_adaptive_list,
                                             ATTENTION_METHOD, square_size, square_att_coordinates_list,
                                             square_att_coordinates_adaptive_list, original_svg_points_list,
                                             original_svg_points_adaptive_list, mutated_points_att_list_numeric,
                                             mutated_points_att_adaptive_list_numeric, ext_att_list,
                                             ext_att_adaptive_list, ext_normal_list, ext_normal_adaptive_list)
                        make_gif(folder_path, folder_path + "/gif")
                    ...
                    with open(csv_path_2, "a") as f1:
                        writer = csv.writer(f1)
                        writer.writerow([image_index, LABEL, repetition, seed, iterations_detection_att,
                                         iterations_detection_att_adaptive, iterations_detection_normal,
                                         iterations_detection_normal_adaptive, method_winner])
            ...
            with open(csv_path_3, "a") as f1:
                ...
                writer.writerow(
                    [image_index, LABEL, iterations_mean_att, iterations_mean_att_adaptive, iterations_mean_normal,
                     iterations_mean_normal_adaptive, iterations_std_att, iterations_std_att_adaptive,
                     iterations_std_normal, iterations_std_normal_adaptive, number_of_miss_classification_att,
                     number_of_miss_classification_att_adaptive, number_of_miss_classification_normal,
                     number_of_miss_classification_normal_adaptive])

            save_boxPlots_adaptive2(iterations_mean_att_list, iterations_mean_att_adaptive_list,
                                    iterations_mean_normal_list, iterations_mean_normal_adaptive_list,
                                    number_of_miss_classification_att_list,
                                    number_of_miss_classification_att_adaptive_list,
                                    number_of_miss_classification_normal_list,
                                    number_of_miss_classification_normal_adaptive_list, DST, "ADAPTIVE",
                                    NUMBER_OF_MUTATIONS, NUMBER_OF_REPETITIONS)
            print("iterations_mean_att_list: ", iterations_mean_att_list)
            print("iterations_mean_att_adaptive_list: ", iterations_mean_att_adaptive_list)
            print("iterations_mean_normal_list: ", iterations_mean_normal_list)
            ...
            print("number_of_miss_classification_att_adaptive_list: ", number_of_miss_classification_att_adaptive_list)
            print("number_of_miss_classification_normal_list: ", number_of_miss_classification_normal_list)
            print("number_of_miss_classification_normal_adaptive_list: ",
                  number_of_miss_classification_normal_adaptive_list)
